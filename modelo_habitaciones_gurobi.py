...
def generar_feedback(instancia, pacientes_planificados, conflict_days, ruta_feedback,
                     motivo="habitaciones", reduccion_fraccion=0.20):
    """
    Genera feedback para la fase de admisión.

    Reduce el número de admisiones de los días conflictivos en un porcentaje
    (reduccion_fraccion, defecto 20 %) en lugar de solo 1. Esto acelera la
    convergencia del bucle de feedback en instancias grandes donde la
    reducción de 1 en 1 puede necesitar decenas de iteraciones.
    ...
    for d in sorted(conflict_days):
        ...
        if current > 0:
            # Reducir al menos 1 y hasta reduccion_fraccion del total
            reduccion = max(1, math.ceil(current * reduccion_fraccion))
            day_caps[str(d)] = max(0, current - reduccion)
            day_penalties[str(d)] = 10.0

        for g, mapa in admisiones_por_genero_dia.items():
            current_g = mapa.get(d, 0)
            if current_g > 0:
                reduccion_g = max(1, math.ceil(current_g * reduccion_fraccion))
                gender_caps.setdefault(g, {})
                gender_penalties.setdefault(g, {})
                gender_caps[g][str(d)] = max(0, current_g - reduccion_g)
