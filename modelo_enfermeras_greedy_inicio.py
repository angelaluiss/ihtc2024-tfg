...
def get_weights(instance):
    ...
    # Las claves REALES en las instancias IHTC son:
    #   room_nurse_skill, continuity_of_care, nurse_eccessive_workload
    # (nota: "eccessive" está así escrito en el formato oficial).
    # Se mantienen alias alternativos por robustez.
    return {
        "room_skill_level": w.get("room_nurse_skill",
                              w.get("room_skill_level", w.get("nurse_skill", 1))),
        "continuity_of_care": w.get("continuity_of_care", 5),
        "excessive_nurse_workload": w.get("nurse_eccessive_workload",
                                     w.get("excessive_nurse_workload", w.get("excessive_workload", 1))),
    }
...
def value_from_profile(profile, relative_day, shift, shift_index=None, n_shifts=3, default=0):
    """
    Extrae un valor de un perfil temporal con diferentes posibles formatos.

    Formato oficial IHTC (el que usan las instancias reales):
      0) lista PLANA de longitud n_shifts*length_of_stay, ordenada por
         (día relativo, turno). El valor del turno (relative_day, shift_index)
         está en la posición relative_day*n_shifts + shift_index.

    Otros formatos soportados por robustez:
    ...
    if isinstance(profile, list):
        ...
        # Formato oficial IHTC: lista plana de escalares indexada por turno.
        # IMPORTANTE: hay que usar el índice rel_day*n_shifts+shift_index;
        # indexar solo por relative_day ignora el turno y lee valores erróneos.
        if shift_index is not None and all(isinstance(x, (int, float)) for x in profile):
            idx = relative_day * n_shifts + shift_index
...
def get_workload(entity, relative_day, shift, shift_index=None, n_shifts=3):
    """
    ...
    return float(value_from_profile(profile, relative_day, shift, shift_index, n_shifts=n_shifts, default=0))


def get_required_skill(entity, relative_day, shift, shift_index=None, n_shifts=3):
    """
    ...
    return int(value_from_profile(profile, relative_day, shift, shift_index, n_shifts=n_shifts, default=0))
...
def construir_datos_enfermeria(instance, solution):
        ...
        for room in room_ids:
            ...
            for t in shift_types:
                ...
                for kind, eid, rel_day in entities:
                    ...
                    total_workload += get_workload(ent, rel_day, t, shift_index[t], n_shifts=n_shifts)
                    skill_ent = get_required_skill(ent, rel_day, t, shift_index[t], n_shifts=n_shifts)
                    skills_individuales.append(skill_ent)
                    required_skill = max(required_skill, skill_ent)
                    entity_ids.append(eid)

                occupied_rooms[(d, t)].append(room)
                room_workload[(d, t, room)] = total_workload
                # Máximo requerido en la habitación: solo para ordenar dificultad.
                room_required_skill[(d, t, room)] = required_skill
                # Lista de niveles individuales: el coste de cualificación del IHTC
                # se suma POR paciente/ocupante, no se mide por habitación.
                room_required_skills_list[(d, t, room)] = skills_individuales
    ...
    return {
        ...
        "room_required_skills_list": room_required_skills_list,
...
def evaluar_assignment(datos, assignment):
    ...
    for (d, t), rooms in datos["occupied_rooms"].items():
        for room in rooms:
            ...
            # Coste de cualificación: se suma el déficit POR cada paciente/ocupante
            # presente en la habitación (definición oficial IHTC), no el déficit
            # respecto al máximo de la habitación.
            for req_skill in datos["room_required_skills_list"].get(key, []):
...
def coste_incremental_greedy(datos, assignment, nurse_load, entity_nurses, d, t, room, nurse):
    ...
    # Déficit de cualificación sumado por paciente/ocupante de la habitación.
    skill_deficit = sum(
        max(0, req_skill - n_skill)
        for req_skill in datos["room_required_skills_list"].get((d, t, room), [])
    )
